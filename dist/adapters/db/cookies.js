...
const zSavedCookie = zod_1.z
    ...
    .array();
const saveLoginCookies = async (fileName, cookies) => {
    const bucket = infrastructure_1.storageClient.bucket(infrastructure_1.bucketName.cookies);
...
const retrieveLoginCookies = async (fileName) => {
    const bucket = infrastructure_1.storageClient.bucket(infrastructure_1.bucketName.cookies);
    const file = bucket.file(`login_cookies_${fileName}.json`);
